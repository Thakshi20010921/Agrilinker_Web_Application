...
export default function FertilizerList() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-8">
        ...
        {currentItems.map((f) => {
          ...
          return (
            <div key={id} className="bg-white p-5 rounded-xl shadow hover:shadow-lg transition flex flex-col justify-between">
              <div>
                <img src={f.imageUrl || "https://via.placeholder.com/300x200"} alt={f.name} className="rounded-lg mb-4 w-full h-48 object-cover" />
                ...
                <p className="text-sm text-gray-600 mb-1">Type: <span className="font-semibold">{f.type}</span></p>
                <p className="text-sm text-gray-600 mb-1">Category: <span className="font-semibold">{f.category}</span></p>
                <p className="text-sm text-gray-600 mb-2">District: <span className="font-semibold">{f.district}</span></p>
              ...
              <div className="flex justify-between mt-4">
                <Link to={`/fertilizers/edit/${id}`} className="bg-yellow-500 text-white px-4 py-2 rounded-lg hover:bg-yellow-600 transition">Edit</Link>
                <button onClick={() => handleBuy(f)} className="bg-green-700 text-white px-4 py-2 rounded-lg hover:bg-green-800 transition">Buy</button>
              </div>
            </div>
          );
        })}
        ...
        <div className="flex justify-center mt-10 gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
            <button
              ...
              className={`px-4 py-2 border rounded-lg ${currentPage === page ? "bg-green-700 text-white" : "bg-white text-gray-700 hover:bg-gray-100"}`}
            >
              {page}
            </button>
          ))}
      ...
      {/* Review Modal */}
      {selectedItem && (
        <ReviewModal item={selectedItem} itemType={reviewType} onClose={() => setSelectedItem(null)} />
      )}
    </div>
  );
}
